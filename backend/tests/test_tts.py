...
async def test_synthesize_returns_bytes():
    ...
    with patch("src.voice_generation.tts_engine.edge_tts.Communicate", return_value=mock_communicate):
        from src.voice_generation.tts_engine import synthesize
...
async def test_synthesize_with_timing_returns_word_boundaries():
    ...
    with patch("src.voice_generation.tts_engine.edge_tts.Communicate", return_value=mock_communicate):
        from src.voice_generation.tts_engine import synthesize_with_timing
...
def test_build_narration_segments_uses_tts_timeline():
    from src.voice_generation.tts_engine import build_narration_segments
...
async def test_list_voices_returns_formatted_list():
    ...
    with patch("src.voice_generation.tts_engine.edge_tts.list_voices", new_callable=AsyncMock) as mock:
